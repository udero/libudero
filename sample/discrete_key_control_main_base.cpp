...
int onInitialized(const int argc, const char* argv[]) {
  ...
  if (initKeyInput(loader)) {
    ...
    base = udero_base_init(udero);
    if (base == NULL) {
      UERROR("onInitialized failed.");
      return -1;
    }
...
int onExecuted() {
  ...
  cmd = moveUdero(udero, base, cmd);
  cmd = moveUderoBase(base, cmd);
  udero->spin();
  base->spin();


  updateUdero(udero);
  updateUderoBase(base);
  if (update_display) {
