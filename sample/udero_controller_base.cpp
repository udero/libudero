...
void updateUderoBase(reharo::IUderoBase* base) {
  basePos = base->getJointPosition(0);
}
...
void moveJointBase(reharo::IUderoBase* base, double step, double joint_vel) {
  base->moveJoint(0, basePos + step, joint_vel);
}
...
void foldOut(reharo::IUdero* udero_, reharo::IUderoBase* base) {
  UINFO("moveMode == MOVE_FOLDOUT");
  for(int i = 0;i < 6;i++) {
    udero_->setJointMode(i, reharo::MODE_POSITION);
  }
  base->setJointMode(0, reharo::MODE_POSITION);

  udero_->foldOut();
  // base->moveJoint(0, 0.01, 0.1);
  std::vector<double> pos;
  std::vector<double> vel;
  for (int i = 0; i < 6; i++) {
...
MoveCommand moveUdero(reharo::IUdero* udero_, reharo::IUderoBase* base, const MoveCommand& command) {
  ...
  if (moveMode_ == MOVE_MOVING) {
    ...
    for(int i = 0;i < 6;i++) {
      udero_->setJointMode(i, reharo::MODE_INACTIVE);
    }
    base->setJointMode(0, reharo::MODE_INACTIVE);
...
reharo::IUderoBase* udero_base_init(reharo::IUdero* udero) {
  UTRACE("udero_base_init called");
  reharo::IUderoBase* base = createUderoBase(udero);
  if (!base) {
    UERROR("udero_base_init failed.");
    return NULL;
  }
  base->setJointMode(0, reharo::MODE_POSITION);
  return base;
...
void udero_base_fini(reharo::IUderoBase* base) {
  deleteUderoBase(base);
}

MoveCommand moveUderoBase(reharo::IUderoBase* base, const MoveCommand& command) {
  const ControlSpeed& spd = command.speed;
  MOVE_MODE moveMode_ = command.moveMode;
  MOVING_DIRECTION movingDirection_ = command.movingDirection;
  UTRACE("moveUdero called");
  if (moveMode_ == MOVE_MOVING) {
    UINFO("moveMode_ == MOVE_MOVING");
    switch (movingDirection_) {
    case MOVING_BASE_POSITIVE:
      base->setJointMode(0, reharo::MODE_POSITION);
      moveJointBase(base,  spd.joint_step / 180.0 * M_PI, spd.joint_vel / 180.0 * M_PI * 0.5 );
      break;
    case MOVING_BASE_NEGATIVE:
      base->setJointMode(0, reharo::MODE_POSITION);      
      moveJointBase(base,  -spd.joint_step / 180.0 * M_PI, spd.joint_vel / 180.0 * M_PI * 0.5);
      break;
    default:
